...
async def create_subject(subject: SubjectCreate, db: AsyncSession = Depends(get_async_db)):
...
async def get_subject_teachers(subject_id: int, db: AsyncSession = Depends(get_async_db)):
...
@router.post("/{subject_id}/teachers",
             ...
             status_code=201)
async def create_subject_teacher(subject_id: int, user_id: str, db: AsyncSession = Depends(get_async_db)):
...
async def delete_subject_teacher(subject_id: int, user_id: str, db: AsyncSession = Depends(get_async_db)):
