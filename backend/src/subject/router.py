...
@router.post("/", response_model=Subject,
             dependencies=[Depends(admin_user_validation)], status_code=201)
async def create_subject(subject: SubjectCreate, db: Session = Depends(get_async_db)):
    return await service.create_subject(db, subject)


@router.delete("/{subject_id}",
               dependencies=[Depends(admin_user_validation)], status_code=200)
async def delete_subject(subject_id: int, db: Session = Depends(get_async_db)):
...
async def get_subject_teachers(subject_id: int, db: Session = Depends(get_async_db)):
...
@router.post("/{subject_id}/teachers",
             ...
             status_code=201)
async def create_subject_teacher(subject_id: int, user_id: str, db: Session = Depends(get_async_db)):
...
async def delete_subject_teacher(subject_id: int, user_id: str, db: Session = Depends(get_async_db)):
